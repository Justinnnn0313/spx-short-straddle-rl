...
class ActorCriticExitPolicy:
    def __init__(self, model_path: str | Path, threshold: float = 0.5):
        state = torch.load(model_path, map_location="cpu", weights_only=False)
        self.model = ExitActorCritic(int(state["input_dim"]), int(state["hidden_dim"]))
    ...
    def compute_single_action(self, obs, explore: bool = False) -> int:
        ...
        if mask is not None and (len(mask) <= EXIT or mask[EXIT] == 0):
            return WAIT
        x = (features.reshape(1, -1) - self.mean) / self.std
        with torch.no_grad():
            logit, _value = self.model(torch.from_numpy(x.astype(np.float32)))
...
def patch_policy_loader(model_path: str, threshold: float) -> None:
    ...
    class _PatchedPolicy(ActorCriticExitPolicy):
        def __init__(self):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate fine-tuned actor-critic exit policy in portfolio OOS.")
    ...
    parser.add_argument("--plot-title", default="BC-initialized Actor-Critic Exit OOS")
...
def main() -> dict:
    ...
    portfolio_args = argparse.Namespace(
        ...
        model_variant="exit",
        policy_loader="actor_critic_finetuned",
    ...
    if summary_path.exists():
        ...
        data["bc_initialized_actor_critic_model_path"] = args.model_path
        data["policy_threshold"] = args.threshold
